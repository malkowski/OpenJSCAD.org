 ...
 *
 * Example:
 *  solid = shapes.solidFromSlices([ {id:"layer1",transform:[0,0,0]}, {id:"layer2",transform:[0,0,10]}, {id:"layer3",transform:[0,0,12]} ])
 */
...
FlatShapeCollection.prototype.rotateSlicePath = function (id) {
	...
	function getAngleFromTriangle (w, h) {
		//var angle = (w<=0 && h<=0 ? 0 : 180) - Math.atan(Math.abs(w)<Math.abs(h) ? w/h : h/w)/Math.PI*180;
		var angle = (w<=0 && h<=0 ? 0 : 180) - Math.atan(h/w)/Math.PI*180;
