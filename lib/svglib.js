...
FlatShapeCollection.prototype.extrude = function (id, height, axis) {
	...
	if (typeof arguments[0] == "object") {
		axis   = ( arguments[0].axis   !== undefined ? arguments[0].axis   : undefined )
		height = ( arguments[0].height !== undefined ? arguments[0].height : undefined )
		fn     = ( arguments[0].fn     !== undefined ? arguments[0].fn     : this.fn   )
		id     = ( arguments[0].id     !== undefined ? arguments[0].id     : undefined )
	}
	...
	if (id instanceof Array) {
		var out = union(id.map(function(id){
			return self.extrude(id, height, axis);
		}))
	}
	else {
		var out = this.createShapeFromSvgData(this.shapes[id], height, axis)
	}
	this.fn = oldfn
	return out
};

function __validateXYZlayers (id) {
	if (!shapes.shapes[id]) {
		throw new Error(`${id} could not be found`)
	}
	if (!shapes.shapes[id+"Z"]) {
		throw new Error(`${id}Z could not be found`)
	}
}

FlatShapeCollection.prototype.extrudeXY = function (id) {
	__validateXYZlayers(id)
    var refZ = shapes.extrude(id+"Z",1)
    var boundsZ = t.getBounds(refZ)
    var refXY = shapes.extrude(id, boundsZ.s.y)
    return refXY
    .translate([ 0, 0, boundsZ.b[0].y ])
}

FlatShapeCollection.prototype.extrudeZ = function (id) {
	__validateXYZlayers(id)
    var ref = shapes.extrude(id, 1)
    var bounds = t.getBounds(ref)
    var refZ = shapes.extrude(id+"Z", bounds.s.y)
    var boundsZ = t.getBounds(refZ)
    return refZ
    .rotateX(90)
    .translate([ 0, bounds.b[1].y, 0 ])
}

FlatShapeCollection.prototype.rotateXY = function (id) {
	__validateXYZlayers(id)
    var refZ = shapes.extrude(id+"Z",1)
    var boundsZ = t.getBounds(refZ)
    return shapes.rotate(id, boundsZ.s.y)
    .translate([ 0, 0, boundsZ.b[0].y + boundsZ.s.y/2 ])
}
 ...
 * layers = [ { id: "", translate: [x,y,z], rotate:[aX,aY,aZ], transform: function }, ... ]
 *
 * Example:
 *  solid = shapes.solidFromSlices([ {id:"layer1",transform:function(csg){ ... }}, {id:"layer2",translate:[0,0,10]}, {id:"layer3",translate:[0,0,12]} ])
 */
FlatShapeCollection.prototype.solidFromSlices = function (layers) {
	...
	return CSG.fromSlices({
		...
		callback: function (t,slice) {
			...
			if (layer.transform) csg = layer.transform(csg)
...
svglib._getOffsetForAngle = function (a, r) {
    return [
        Math.sin(((a+720)%360)/180*Math.PI)*r,
        Math.cos(((a+720)%360)/180*Math.PI)*r,
    ]
}

FlatShapeCollection.prototype.createShapeFromAngledPoints = function (args) {
	var id = args.id
	var angledPoints = args.angledPoints
	var radius = args.radius

	var leftPoints = []
	var rightPoints = []

	var dotL = t.ccube([ 0.001, 0.001, 1 ]).translate([ -radius, 0, 0 ]) 
	var dotR = t.ccube([ 0.001, 0.001, 1 ]).translate([  radius, 0, 0 ]) 
	var dots = []
	angledPoints.forEach(function(l){
		var a = l.angle
		var p = l.point

		var myDotL = dotL.rotateZ(a).translate([ p[0], p[1], 0 ])
		dots.push(myDotL)
		var boundsL = t.getBounds(myDotL)

		var myDotR = dotR.rotateZ(a).translate([ p[0], p[1], 0 ])
		dots.push(myDotR)
		var boundsR = t.getBounds(myDotR)

		leftPoints.push([ boundsL.c.x, boundsL.c.y ])
		rightPoints.unshift([ boundsR.c.x, boundsR.c.y ])
...
/*
	angledPoints.forEach(function(l){
	    ...
	    var size0 = svglib._getOffsetForAngle(a+0, 10)
	    var size1 = svglib._getOffsetForAngle(a+180, 10)
		...
		leftPoints.push(p0)
		rightPoints.unshift(p1)

	})

*/
	var pathPoints = leftPoints.concat(rightPoints)
...
/**
 * During the "create a flat circle slice" step, expand radius 
 * so as to create a larger rotated version of the shape.
 * 
 */
FlatShapeCollection.prototype.expandRotateSlicePath = function (args, radiusPadding) {
	origSliceTransform = false
	if (args.sliceTransform) origSliceTransform = args.sliceTransform

	args.sliceTransform = function (t, i, c) {
		var s = (c.radius+radiusPadding)/c.radius
		var out = this.scale([ s, s, s ])
		if (origSliceTransform) return origSliceTransform.apply(out, [t,i,c])
		return out
	}
    return this.rotateSlicePath(args)
}

/**
 * Same as expandRotateSlicePath, except the axes are aligned the same as extruded shapes.
 * Makes it easier to work with a mixture of both extruded and rotated shapes.
 * (This is twin brother of shapes.rotate function)
 */
FlatShapeCollection.prototype.expandRotate = function (args, radiusPadding) {
	return this.expandRotateSlicePath(args, radiusPadding).rotateX(-90)
};
...
FlatShapeCollection.prototype.rotateSlicePath = function (id) {
	...
	var sliceTransform;
	var extrusionShape;
	var extrusionShapeScaleUsingAxis = false; // either "X" or "Y", otherwise both X and Y axes are scaled to fit
	if (typeof arguments[0] == "object") {
		fn = arguments[0].fn || t.fn;
		sliceTransform = arguments[0].sliceTransform;
		extrusionShape = arguments[0].extrusionShape;
		extrusionShapeScaleUsingAxis = arguments[0].extrusionShapeScaleUsingAxis;
	...
	pointSets.forEach(function(ps,i){
		...
		var angleY = svglib.getAngleFromTriangle(triangle.width, triangle.height);
	...
	var circlePoints = (function(){
		...
		if (extrusionShape) {
			...
			var sliceScale = [ 2/size.x, 2/size.y, 1 ];
			if (extrusionShapeScaleUsingAxis && extrusionShapeScaleUsingAxis.toLowerCase) switch (extrusionShapeScaleUsingAxis.toUpperCase()) {
				case "X":
					sliceScale[1] = sliceScale[0]
				break;
				case "Y":
					sliceScale[0] = sliceScale[1]
				break;
			}
			var shape = csg
...
/**
 * Given an open-ended U-shaped polyline, convert each set of points into a square,
 * starting with first/last point, and ending with middle two points.
 * Turns 2D shape into a rotateExtrude-like shape, keeping each slice in the correct position.
 * 
 * Cloned from rotateSlicePath, modified to be a square instead of circle.
 */
FlatShapeCollection.prototype.squareSlicePath = function (id) {
	var self = this;
	...
	pointSets.forEach(function(ps,i){
		...
		if (maxWidth < radius*2) maxWidth = radius*2
	...
	var circlePoints = (function(){
		...
		if (extrusionShape) {
			...
			var cagCircle = ( cornerR ? new CAG.roundedRectangle({ radius: [ maxWidth/2, maxWidth/2 ], roundradius: cornerR, resolution: fn }).scale([ 1/(maxWidth/2), 1/(maxWidth/2) ]) : new CAG.rectangle() )
...
/**
 * If this code has turned into a hot mess, trash it.
 * Fork of prototype.followPath, which was useful for one particular thing but had too many pecularities.
 *
 * options: {
 *     pathShapeId: this is the path we'll be using as a starting point
 *     templateShapeId: this shape is the pattern that's used at each point in the path
 *     roundEnds: put half-spheres on each end. Only applies to open-ended paths
 *     closedPath: if set to true, well, duh
 * }
 */
FlatShapeCollection.prototype.expandPathUsingTemplate = function (options) {
/*
	if (options.roundEnds  === undefined) options.roundEnds  = true
	if (options.closedPath === undefined) options.closedPath = false
	if (options.fn         === undefined) options.fn         = this.fn

	var templateShape = this.extrude(options.templateShapeId, 1)
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	var numSlices = (options.fn ? options.fn : this.fn)+1

	var pathPoints = shapes.shapes[options.pathShapeId].attr.points
		.map(function(p){ return new CSG.Vector2D(p) })
	...
	.solidFromSlices({
		...
		callback: function (v,layer) {
			...
			var layerPoints = v==1 ? pathPoints : pathPoints.map(function(p,i){

				var prevP = pathPoints[ i==0 ? pathPoints.length-1 : i-1 ]
				var nextP = pathPoints[ i+1==pathPoints.length ? 0 : i+1 ]
 ...
 * 
 * Here's a great example of the same sort of idea, done much more effectively, with way less garbage in the results:
 * 20181222 old classic, new material/attachmentNegativeMold.jscad (specifically, the extrudePathWithTemplateShape function)
 */
...
FlatShapeCollection.prototype.getPointsAndAnglesFromPathAsync = function (args) {
	var id = args.id
	var pointDistance = args.pointDistance
	var padStart = args.padStart || 0 // instead of placing first point at 0 on the line, we place it this far along
	var padEnd = args.padEnd || 0 // ^^ same as above but for the opposite end
	var callback = args.callback

	this.getPointsFromPathAsync({
		id: id,
		...
		callback: function(points){
			...
			angledPoints.push({
			    point: points[0],
			    angle: svglib.findAngleBetweenVectors(points[0], points[1])
			})
			...
			for (var i=1; i<points.length-1; i++) {
	            ...
	            var prevAngle = svglib.findAngleBetweenVectors(prevPoint, thisPoint)
	            var nextAngle = svglib.findAngleBetweenVectors(thisPoint, nextPoint)
	            var thisAngle = ( (prevAngle+720)%360 + (nextAngle+720)%360 )/2
	        
	            prevPoint = new CSG.Vector2D(prevPoint)
	            nextPoint = new CSG.Vector2D(nextPoint)
	        
	            /*
	            var size = prevPoint.max(nextPoint).minus(prevPoint.min(nextPoint))
	            console.log("size: [ " + Math.round(size.x) + ", " + Math.round(size.y) + " ]")
	            console.log("ratio: " + (size.x/size.y).toFixed(3) + "")
	        
				console.log("calculated angles: [ " + [
	                Math.round(prevAngle),
	                Math.round(thisAngle),
	                Math.round(nextAngle),
	            ].join(", ") + " ]")
				*/
	        
	            angledPoints.push({ point: thisPoint, angle: thisAngle })
	
			}
	
			angledPoints.push({
			    point: points[points.length-1],
			    angle: svglib.findAngleBetweenVectors(points[points.length-2], points[points.length-1])
			})
	
			return callback(angledPoints)
		}
	})
...
FlatShapeCollection.prototype.getPointsFromPathAsync = function (args) {
    var self = this;

	var id = args.id
	var pointDistance = args.pointDistance
	var padStart = args.padStart // instead of placing first point at 0 on the line, we place it this far along
	var padEnd = args.padEnd  // ^^ same as above but for the opposite end
	var callback = args.callback
...
/**
 * The findAngleBetweenVectors function probably doesn't work right, seeing as how I eventually figured *this* out but didn't realize this other function existed.
 * Fuck it, probably breaks a bunch of stuff if I go trying to fix it now.
 *
 * So here's the one that works well, used heavily in rotateSlicePath.
 *
 */
svglib.getAngleFromTriangle = function (w, h) {
