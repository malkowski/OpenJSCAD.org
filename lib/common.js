...
// same syntax as global 'union' etc. from web interface, for convenience.
t.union = function(csg){         return csg.shift().union(csg);     };
t.intersection = function(csg){  return csg.shift().intersect(csg); };
t.difference = function(csg){    return csg.shift().intersect(csg); };
...
t.ccube = function (size) {
    return CSG.cube(size).translate([-size[0]/2,-size[1]/2,0])
};
...
//    return t.sccube({ r: r, fn: fn, size: size, z: [0,0] });
    ...
    shapes.push(
        CSG.cylinder({ r:r, h:size[2], fn: fn }).translate([   size[0]/2-r,    size[1]/2-r,  0 ]),
        CSG.cylinder({ r:r, h:size[2], fn: fn }).translate([ -(size[0]/2-r),   size[1]/2-r,  0 ]),
        CSG.cylinder({ r:r, h:size[2], fn: fn }).translate([   size[0]/2-r,  -(size[1]/2-r), 0 ]),
        CSG.cylinder({ r:r, h:size[2], fn: fn }).translate([ -(size[0]/2-r), -(size[1]/2-r), 0 ])
    );

    return t.union(shapes);
};
...
t.sccube = function (params) {
    ...
    shapes.push(
        CSG.cylinder({ r:r, h:size[2]-r*(z[0]+z[1]), fn: fn }).translate([   size[0]/2-r,    size[1]/2-r,  r*z[0] ]),
        CSG.cylinder({ r:r, h:size[2]-r*(z[0]+z[1]), fn: fn }).translate([ -(size[0]/2-r),   size[1]/2-r,  r*z[0] ]),
        CSG.cylinder({ r:r, h:size[2]-r*(z[0]+z[1]), fn: fn }).translate([   size[0]/2-r,  -(size[1]/2-r), r*z[0] ]),
        CSG.cylinder({ r:r, h:size[2]-r*(z[0]+z[1]), fn: fn }).translate([ -(size[0]/2-r), -(size[1]/2-r), r*z[0] ])
    );
    ...
    if (z[0]) {
        shapes.push(
            CSG.cylinder({ r:r, h:size[1]-r*2, fn: fn }).rotateX(90).translate([ -(size[0]/2-r),   size[1]/2-r,  r         ]),
            CSG.cylinder({ r:r, h:size[1]-r*2, fn: fn }).rotateX(90).translate([   size[0]/2-r,    size[1]/2-r,  r         ]),
            CSG.cylinder({ r:r, h:size[0]-r*2, fn: fn }).rotateY(90).translate([ -(size[0]/2-r),   size[1]/2-r,  r         ]),
            CSG.cylinder({ r:r, h:size[0]-r*2, fn: fn }).rotateY(90).translate([ -(size[0]/2-r), -(size[1]/2-r), r         ]),
            CSG.sphere({ r:r, fn:fn }).translate([   size[0]/2-r,    size[1]/2-r,  r ]), 
            CSG.sphere({ r:r, fn:fn }).translate([ -(size[0]/2-r),   size[1]/2-r,  r ]), 
            CSG.sphere({ r:r, fn:fn }).translate([   size[0]/2-r,  -(size[1]/2-r), r ]), 
            CSG.sphere({ r:r, fn:fn }).translate([ -(size[0]/2-r), -(size[1]/2-r), r ]) 
        );
    ...
    if (z[1]) {
        shapes.push(
            CSG.cylinder({ r:r, h:size[0]-r*2, fn: fn }).rotateY(90).translate([ -(size[0]/2-r),   size[1]/2-r,  size[2]-r ]),
            CSG.cylinder({ r:r, h:size[0]-r*2, fn: fn }).rotateY(90).translate([ -(size[0]/2-r), -(size[1]/2-r), size[2]-r ]),
            CSG.cylinder({ r:r, h:size[1]-r*2, fn: fn }).rotateX(90).translate([ -(size[0]/2-r),   size[1]/2-r,  size[2]-r ]),
            CSG.cylinder({ r:r, h:size[1]-r*2, fn: fn }).rotateX(90).translate([   size[0]/2-r,    size[1]/2-r,  size[2]-r ]),
            CSG.sphere({ r:r, fn:fn }).translate([   size[0]/2-r,    size[1]/2-r,  size[2]-r ]), 
            CSG.sphere({ r:r, fn:fn }).translate([ -(size[0]/2-r),   size[1]/2-r,  size[2]-r ]), 
            CSG.sphere({ r:r, fn:fn }).translate([   size[0]/2-r,  -(size[1]/2-r), size[2]-r ]), 
            CSG.sphere({ r:r, fn:fn }).translate([ -(size[0]/2-r), -(size[1]/2-r), size[2]-r ]) 
        );
...
t.reflectCylinderAtCornersQuad = function (arguments) {
    ...
    var cornerShape = difference(
        ...
        CSG.cube(arguments.size),
        CSG.cylinder(arguments.corner).scale([1,1,arguments.size[2]])
    ).translate([
