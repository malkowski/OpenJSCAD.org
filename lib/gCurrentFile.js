...
$(function(){
	...
	$("#editorPanel").tabs({
		...
		activate: function () {
			var tabId = $(this).tabs('option', 'active');
			var activeTab = tabs[tabId];
			activeTab && activeTab.editor && activeTab.editor.focus();
		},
	});
	...
	tabs.push({ editor: gEditor });

    gEditor.commands.addCommand({
       name: 'setJsCad',
       bindKey: { win: 'Shift-Return', mac: 'Shift-Return' },
       exec: function(editor) {
          gProcessor.prepareForRender && gProcessor.prepareForRender();
    ...
    gEditor.commands.addCommand({
      ...
      exec: function (editor) {
        gProcessor.sendCurrentSourceCode();
      }
    });

    gProcessor.onreload = function () {
